...
def collate_fn_eval(batch):
    ...
    input_ids, type_id, attr_id, attention_masks, attr_index, token_index = list(zip(*batch))
    ...
    type_id = [torch.tensor(instance) for i, instance in enumerate(type_id)]
    attr_id = [torch.tensor(instance) for i, instance in enumerate(attr_id)]
    attr_index = [torch.tensor(instance).long() for i, instance in enumerate(attr_index)]
    token_index = [torch.tensor(instance).long() for i, instance in enumerate(token_index)]
    ...
    type_id = rnn_utils.pad_sequence(type_id, batch_first=True, padding_value=0) # pad_id
    attr_id = rnn_utils.pad_sequence(attr_id, batch_first=True, padding_value=0) # pad_id
    attr_index = rnn_utils.pad_sequence(attr_index, batch_first=True, padding_value=0) # pad_id
    token_index = rnn_utils.pad_sequence(token_index, batch_first=True, padding_value=0) # pad_id

    return input_ids, input_masks, type_id, attr_id, attr_index, token_index
...
def collate_fn_v1(batch):
    ...
    input_ids, type_id, attr_id, attention_masks, attr_index, token_index, attr_label, token_label = list(zip(*new_batch))
    ...
    attr_index = [torch.tensor(instance).long() for i, instance in enumerate(attr_index)]
    token_index = [torch.tensor(instance).long() for i, instance in enumerate(token_index)]
    attr_label = [torch.tensor(instance) for i, instance in enumerate(attr_label)]
    token_label = [torch.tensor(instance) for i, instance in enumerate(token_label)]
    ...
    attr_index = rnn_utils.pad_sequence(attr_index, batch_first=True, padding_value=0) # pad_id
    token_index = rnn_utils.pad_sequence(token_index, batch_first=True, padding_value=0) # pad_id
    attr_label = rnn_utils.pad_sequence(attr_label, batch_first=True, padding_value=-100) # pad_id
    token_label = rnn_utils.pad_sequence(token_label, batch_first=True, padding_value=-100) # pad_id

    return input_ids, input_masks, type_id, attr_id, attr_index, token_index, attr_label, token_label
...
def collate_fn(batch):
    ...
    for feature in batch:
        new_batch.extend(feature) 
    # input_ids, attention_masks, outputs, labels = list(zip(*new_batch))
    input_ids, type_id, attr_id, attention_masks, attr_label, token_label = list(zip(*new_batch))
    # input_ids, attention_masks, position_id, outputs, decoder_position_id, labels = list(zip(*new_batch))
    ...
    attr_label = torch.tensor(attr_label)
    ...
    # mask_1 = input_ids > 73
    # mask_2 = input_ids < 0
    # indices_1 = torch.nonzero(mask_1).tolist()
    # indices_2 = torch.nonzero(mask_2).tolist()
    # if len(indices_1) > 0 or len(indices_2) > 0:
    #     print('a')
    #     a = input_ids.tolist()

    # return input_ids, input_masks, outputs, labels
    return input_ids, input_masks, type_id, attr_id, attr_label, token_label

def collate_fn_origin(batch):
